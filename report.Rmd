...
* What are the estimated associations between environmental factors (PM2.5, temperature, greenspace, and Heat Vulnerability Index()) and pediatric asthma ED rates, accounting for borough-level and temporal variation?

#### Main Questions

* What is the relationship between environmental factors (PM2.5, temperature, greenspace, Heat Vulnerability Index (HVI)) and pediatric asthma emergencies?
* Which environmental factors show the strongest associations with pediatric asthma emergencies?
* How are these environmental health burdens geographically distributed across New York City?
* Is HVI a good model predictor for pediatric asthma emergencies?
...
# Create initial dataset of acres by type, borough, and time period
...
# Calculate average temperatures by borough and time period
...
mean_temp_year |> 
  ...
  knitr::kable(col.names = c("Time Period", "Bronx", "Brooklyn", "Manhattan", "Queens", "Staten Island"))
...
```
**Figure 8. Mean Heat Vulnerability Index (HVI) by Borough**
...
# Predictor and Outcome Associations

## PM2.5  

```{r pm2.5 file, include=FALSE}
#clean merged file for pm2.5 and outcome comparisons across boroughs and time period
...
**Figure 9. PM2.5 Levels and Outcome (ED Visits Rate and Hospitalization Rate) by Time Period**


```{r}
#combine correlation table
...
knitr::kable(Yr_table, digits = 3, col.names = c("Outcome", "Time Period", "Slope", "R Squared", "r"))
```
**Table 9. Correlation between PM2.5 Levels and Outcomes by Time Period**

**ED Visit Rates**: The association between PM2.5 and ED visit rates are similar for both time periods. Data from 2016 to 2018 and 2019 to 2021 indicate a positive association between the two variables with a correlation coefficient of 0.377 and 0.323, respectively. Thus, PM2.5 levels is weakly associated with hospitalization rate for both time periods.

**Hospitalization Rates**: There is a positive association between PM2.5 and hospitalization rates for both time periods with an estimated correlation coefficient of 0.414. Decreased PM2.5 levels is thus moderately associated with decreased hospitalization rate for both time periods.
...
combo = GS_ED_pm2_5 + GS_HR_pm2_5
combo
```

**Figure 10. PM2.5 Levels and Outcomes across Borough and by Time Period**
...
# Fit linear models for each time period and extract results
...
GS_combo
```
**Figure 11. Greenspace and Outcome (ED Visits Rate and Hospitalization Rate) by Time Period**
...
knitr::kable(Yr_table_GS, digits = 3, col.names = c("Outcome", "Time Period", "Slope", "R Squared", "r"))
```


**Table 10. Correlation between Greenspace and Outcomes by Time Period**
...
**Figure 12. Greenspace and Outcome across Borough and by Time Period**

In Bronx, there is a dramatic decrease in ED visit rate with an increase in total acres of greenspace between the two time period. This decrease is also seen for Staten Island. The association is less obvious for Brooklyn, Manhattan, and Queens. 
...
```{r}
temp_plot = tempplot + tempplot2

temp_plot
```

**Figure 13. Temperature and Outcome (ED Visits Rate and Hospitalization Rate) by Time Period**
...
knitr::kable(temp_Yr_table, digits = 3, col.names = c("Outcome", "Time Period", "Slope", "R Squared", "r"))
```

**Table 11. Correlation between Temperature and Outcomes by Time Period**

**ED Visit Rate**: The correlation coefficient for the time period are different. Data from 2016 to 2018 indicates a weak negative association between temperature and ED visit rate (r = -0.323). The correlation coefficient for 2019 to 2021 (r = -0.039) indicates no association between these two variables. 

We can infer that temperature is not a significant factor for ED visits.

**Hospitalization Rate**: Data from 2016 to 2018 indicates a weak negative association between temperature and hospitalization rate (r = -0.348). The correlation coefficient for 2019 to 2021 (r = -0.120) indicates a very week negative association between these two variables. 
...
**Figure 14. Temperature and Outcomes across Borough and by Time Period**
...
**Figure 15. Associations between HVI and Outcomes**
...
**Table 12. Statistical Results for ED Visits (2016-2018)**
...
**Table 13. Statistical Results for ED Visits (2019-2021)**
...
**Table 14. Statistical Results for Hospitalizations (2016-2018)**
...
**Table 15. Statistical Results for Hospitalizations (2019-2021)**
...
**Table 16. Statistical Table for Model Diagnostics**
...
Our analysis of pediatric asthma outcomes and environmental factors across New York City boroughs identifies several important findings that highlight significant environmental justice concerns. The Bronx consistently showed the highest rates of both ED visits and hospitalizations across both time periods (2016-2018 and 2019-2021), even as overall rates declined. This persistent disparity reflects historical patterns of disinvestment in the Bronx, where predominantly Black and Hispanic communities have long faced the cumulative burden of poor air quality, limited green space, and systemic underinvestment in environmental resources and infrastructure.

The strong correlation between Heat Vulnerability Index (HVI) and pediatric asthma outcomes (both ED visits and hospitalizations) illustrates how environmental health burdens disproportionately affect historically marginalized communities. Areas with higher HVI scores consistently showed higher rates of pediatric asthma emergencies, with each unit increase in HVI associated with significant increases in both ED visits and hospitalizations. Notably, the Bronx had the highest mean HVI (4.07) compared to other boroughs, reflecting the compounded environmental stressors faced by its residents.

A marked decrease in both ED visits and hospitalizations was observed between 2016-2018 and 2019-2021 across all boroughs. ED visit rates decreased by 43-48% across boroughs, with the Bronx showing the largest reduction (48.0%) while still maintaining the highest absolute rates. Hospitalization rates showed smaller but still substantial decreases of 32-40% across boroughs. While some of this reduction may be attributed to changed healthcare utilization patterns during the COVID-19 pandemic, these decreases coincided with improvements in air quality (PM2.5) and increases in greenspace. However, despite these improvements, significant disparities persist, with the Bronx continuing to experience substantially higher rates than other boroughs, suggesting that more targeted interventions are needed to avert the burden of child asthma in communities of color. 

The strong negative correlation between greenspace availability and asthma outcomes across both time periods suggests that access to green areas may play a protective role in pediatric respiratory health. This relationship is particularly concerning given that the Bronx has the lowest greenspace coverage among all boroughs, despite having the highest asthma rates. While the Bronx saw modest increases in greenspace (119.625 acres), this expansion was primarily in large-scale spaces rather than accessible neighborhood parks that might better serve local communities, and therefore benefit children’s respiratory health. 

Our regression analyses revealed strong associations between borough location, Heat Vulnerability Index (HVI), and pediatric asthma outcomes. After adjusting for HVI, we found consistent disparities between boroughs, with Queens showing significantly lower rates of both ED visits and hospitalizations compared to the Bronx. HVI scores showed significant positive associations with both health outcomes, suggesting that children in areas with higher heat vulnerability experience higher rates of asthma-related emergencies.

While these associations were observed in both time periods, the strength of the relationship between HVI and asthma outcomes decreased during 2019-2021, though the patterns of disparity persisted. These differences emphasize how children in neighborhoods with higher HVI scores consistently experience higher rates of acute asthma episodes requiring emergency care. The health implications of these patterns are significant - frequent ED visits and hospitalizations can disrupt children’s lives, potentially affecting school attendance, ability to participate daily activities, and overall quality of life. In turn, these can contribute to broader social and educational inequities.

This analysis had several key limitations. There were constraints to the planned analyses due to spatial data availability. For many environmental variables, we had data on borough-level variations in exposure, but not on zip code or neighborhood level which could provide further insights and better inform interventions. Additionally, our models could not account for individual-level factors or more granular neighborhood characteristics that might influence asthma outcomes. One of the time periods of interest (2019-2021) overlapped with the COVID-19 pandemic, which likely affected healthcare utilization patterns and potentially confounded our findings. The cross-sectional nature of our analysis prevents us from making causal inferences about the relationships we observed. Further, our models could not account for individual-level factors such as genetic predisposition to asthma, indoor air quality, or specific triggers in homes and schools. Future studies with access to more detailed spatial and temporal data, designed to control for additional relevant factors, could help identify specific areas and populations within boroughs that might benefit most from targeted environmental interventions.

Overall, these findings have potential implications for urban planning and public health policy in New York City. Targeted interventions to increase accessible greenspace, reduce heat vulnerability, and improve air quality in high-risk areas, particularly in historically underserved communities like the Bronx, could help reduce the burden of pediatric asthma. Future research should investigate the specific mechanisms through which environmental factors interact to influence pediatric asthma outcomes, in addition to engaging communities directly in designing effective health interventions.
